...
public class ProjectContentManager : MonoBehaviour
{
    ...
    public async Task LoadObject(IRealityFlowObject em)
    {
    ...
    public async Task SaveObject(IRealityFlowObject em)
    {
    ...
    public async Task UpdateObject(GameObject go)
    {
        if (editMode)
        {
    ...
    public async Task RemoveObject(GameObject go)
    {
        if (editMode) // And user has permission to edit
        {
            IRealityFlowObject robj = sceneObjects.Find((robj) => robj.uuid == go.GetComponent<EditableMesh>().uuid);
