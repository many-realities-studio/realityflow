...
public class SerializableEMInfo : MonoBehaviour
{
    ...
    public SerializableEMInfo() { }

    public SerializableEMInfo(EditableMesh em)
    {
    ...
    public SerializableEMInfo(Mesh m, ShapeType baseShape) // Constructor: takes a mesh and fills out SerializableMeshInfo data structure which basically mirrors Mesh object's parts.
    {
        ...
        colors = new SerializableColor[m.colors.Length];
        for (int i = 0; i < m.colors.Length; i++)
        {
            colors[i].SetColor(m.colors[i]);
        }

        shapeID = getShapeID(baseShape);
    }
    ...
    public Mesh GetMesh()
    {
        ...
        //m.colors = colors.GetColor();
        int temp = 0;
        foreach (SerializableColor color in colors)
        {
            m.colors[temp] = color.GetColor();
            temp++;
        }
